...
{
    validNames.push_back("tomita");
    validNames.push_back("adjlist");
    validNames.push_back("degeneracy");
    validNames.push_back("naive");
    validNames.push_back("newhybrid");
    ...
    double threshold(stod((mapCommandLineArgs.find("--threshold") != mapCommandLineArgs.end()) ? mapCommandLineArgs["--threshold"] : "-0.1"));
    //cout << endl << "Fix threshold: " << threshold << endl;
    ...
    if (argc <= 1 || !isValidAlgorithm(algorithm) || inputFile.empty()) {
        cout << "USAGE: " << argv[0] << " --input-file=<filename> --algorithm=<";
        for (auto& str : validNames)
            cout << str << "|";
        cout << endl;
        return 1;
    ...
    } else if (name == "newhybrid") {
        pAlgorithm = new newHybrid(adjacencyList, threshold);
    } else if (name == "naive") {
        pAlgorithm = new NaiveAlgorithm(adjacencyList);
    } else {
        cout << "ERROR: unrecognized algorithm name " << name << endl;
        return 1;
    }
//#define PRINT_CLIQUES_ONE_BY_ONE
