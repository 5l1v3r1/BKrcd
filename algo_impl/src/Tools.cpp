...
vector<list<int>> readInGraphAdjList(int* n, int* m)
{
    ...
    if(scanf("%d", n)!=1)
    {
        fprintf(stdout, "problem with line 1 in input file\n");
        exit(1);
    }

    if(scanf("%d", m)!=1)
    {
        fprintf(stdout, "problem with line 2 in input file\n");
        exit(1);
    }
...
////        cout << "Read Line: " << line << endl << flush;
        ...
        fprintf(stdout, "ERROR: Problem reading number of vertices and edges in file %s\n", fileName.c_str());
        exit(1);
    }
    ...
    while (i < n) {
        if (!instream.good()  || instream.eof()) {
            fprintf(stdout, "ERROR: Problem reading line %d in file %s\n", i+1, fileName.c_str());
            exit(1);
        }
...
//            cout << u << " " << v << endl;
            ...
            assert(v < n && v > -1);
            if (u==v) {
                fprintf(stdout, "ERROR: Detected loop %d->%d\n", u + 1, v + 1);
                exit(1);
            }
...
vector<list<int>> readInGraphAdjList(int &n, int &m, string const &fileName)
{
    ...
    while(i < m)
    {
        ...
        if (instream.good() && !instream.eof()) {
            ...
            fprintf(stdout, "problem with line %d in input file\n", i+2);
            exit(1);
        }
        assert(u < n && u > -1);
        assert(v < n && v > -1);
        if(u==v)
            fprintf(stdout, "Detected loop %d->%d\n", u, v);
        //assert(u != v);
                            ...
                            int n )
{
    ...
    fprintf(stdout, "%ld maximal cliques, ", cliqueCount);
...
{
    fprintf(stdout, " [Algorithm: %s] ", pAlgorithm->GetName().c_str());

    clock_t start = clock();

    long long const cliqueCount = pAlgorithm->Run(cliques);

    clock_t end = clock();

    if (!outputLatex) {
        fprintf(stdout, "%lld ", cliqueCount);
        fprintf(stdout, "%f\n", (double)(end-start)/(double)(CLOCKS_PER_SEC));
        fprintf(stdout, "-----------------------------------------------------\n");
    } else {
...
vector<int> Tools::ReadMetisOrdering(string const &fileName)
{
    ...
    if (!instream.good()  || instream.eof()) {
        fprintf(stdout, "ERROR: Problem reading line 1 in file %s\n", fileName.c_str());
        exit(1);
    }
