...
NeighborListArray** fake_computeDegeneracyOrderArray(vector<list<int>> const &adjlist, int size)
{
    NeighborListArray** orderingArray = (NeighborListArray**)Calloc(size, sizeof(NeighborListArray*));
    for (int i = 0; i < size; ++i) {
        orderingArray[i] = (NeighborListArray*) new NeighborListArray();
        orderingArray[i]->laterDegree = 0;
        orderingArray[i]->earlierDegree = 0;
        orderingArray[i]->orderNumber = i;
    }

    NeighborListArray* vertexordering = (NeighborListArray*) new NeighborListArray();

    vertexordering->vertex = 0;
    for (int i = 1; i < size; ++i)
        vertexordering->later.push_back(i);
    vertexordering->laterDegree = size-1;
    vertexordering->earlierDegree = 0;
    vertexordering->orderNumber   = 0;

    orderingArray[0] = vertexordering;
